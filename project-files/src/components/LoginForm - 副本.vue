...
const clearForm = () => {
  ...
  formData.value.isAustralSian = false
...
const validateReason = (blur) => {
  ...
  const isAllLetters = /^[A-Za-z\s]+$/.test(reason)
  const isAllDigits = /^\d+$/.test(reason)

  if (!isAllLetters) {
    if (blur) errors.value.reason = 'Reason must only contain English letters.'
  } else if (isAllDigits) {
    if (blur) errors.value.reason = 'Reason cannot be only digits.'
  } else {
    errors.value.reason = null
  }
}
